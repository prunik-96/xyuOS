...
// scancodes for F1/F2 (avoid name clash with enum KEY_F1 in keyboard.h)
#define SC_F1  0x3B
#define SC_F2  0x3C
...
void keyboard_on_irq(void){
  ...
  // F1 / F2 keys -> push enum values defined in keyboard.h
  if(code==SC_F1){ if(!release) push(KEY_F1); return; }
  if(code==SC_F2){ if(!release) push(KEY_F2); return; }
